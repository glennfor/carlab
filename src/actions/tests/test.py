...
def set_motor_speed(motor_index, speed_percent):
    ...
    if motor_index == 1:
        pwm_pin, inA, inB, pwm_obj = M1_PWM_PIN, M1_INA_PIN, M1_INB_PIN, m1_pwm
    elif motor_index == 2:
        pwm_pin, inA, inB, pwm_obj = M2_PWM_PIN, M2_INA_PIN, M2_INB_PIN, m2_pwm
    elif motor_index == 3:
        pwm_pin, inA, inB, pwm_obj = M3_PWM_PIN, M3_INA_PIN, M3_INB_PIN, m3_pwm
    else:
    ...
    # Apply speed via PWM duty cycle (0-100% range)
...
def test_individual_motors(test_duration=1.5, speed=TEST_SPEED):
    ...
    print("If motors are still not spinning, consider changing to the RPi.GPIO library")
